...
### Entscheidung
Die App nutzt eine rollenbasierte Tab-Navigation. Die vorhandenen Bereiche werden in `Uebersicht`, `Packen`, `Kunden`, `Produkte`, `Bestellungen`, `Versand & Pakete`, `Retouren`, `Abo-Boxen`, `Lager & Chargen` und `Mitarbeitende` gegliedert. Der aktive Tab wird ueber `?tab=` in der URL gesteuert und weiterhin serverseitig anhand der Rolle gefiltert.
