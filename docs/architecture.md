...
`NW-008` weist Bestellpositionen serverseitig per FIFO der freigegebenen Charge mit fruehestem MHD zu, sofern genuegend verfügbare Menge vorhanden ist. Verfügbarkeit wird aus produzierter Chargenmenge minus gebuchten Lagerreservierungen berechnet. Das Formular zeigt vor dem Speichern keine konkrete Charge als Vorschlag, weil die korrekte Auswahl von Bestellung, Bestellmenge und B2B-Puffer abhaengt; die konkrete Charge wird erst bei der serverseitigen Speicherung bestimmt. Danach zeigt die Bestellposition sie mit Charge, MHD und reservierter Menge ausdrücklich als `Zugewiesene FIFO-Charge`.
